...
const AIChatWidget: React.FC<AIChatWidgetProps> = ({ products }) => {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Call our internal server API instead of Groq directly
      ...
      if (!response.ok) {
        ...
        if (errorMessage === "Invalid API Key" || errorMessage?.includes("api_key")) {
            errorMessage = "API Key Groq di server tidak valid atau belum terpasang. Kakak perlu cek Environment Variables di Dashboard Vercel.";
        }
