...
const CartModal: React.FC<CartModalProps> = ({ 
  ...
  const [step, setStep] = useState<Step>(directCheckoutItem ? 'invoice' : 'cart');
  ...
  const handleApplyVoucher = async () => {
    ...
    try {
      ...
      // Create Combined Product Strings and Codes for validation
      const validationTargets = itemsToCheckout.map(item => {
        const variant = item.product.variants[item.selectedVariantIndex];
        return {
          fullString: `${item.product.appName} ${variant.type} ${variant.duration}`.toLowerCase(),
          code: item.product.product_code?.toLowerCase()
        };
      });
      
      // Real-time Debugging
      console.log("Validation Targets in Cart:", validationTargets);
      ...
      if (data.product_target && data.product_target.trim() !== '') {
        const target = data.product_target.trim().toLowerCase();
        const hasApplicableProduct = validationTargets.some(t => 
          t.fullString === target || t.code === target
        );
        
        if (!hasApplicableProduct) {
          setVoucherError(`Voucher ini tidak berlaku untuk produk pilihanmu.`);
          return;
        }
      }
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="border border-stone-200 dark:border-white/10 rounded-xl overflow-hidden">
                      <AnimatedButton onClick={() => { setOpenDropdown(openDropdown === 'QRIS' ? null : 'QRIS'); setSelectedPaymentMethod('QRIS'); }} className="w-full p-4 flex justify-between items-center bg-white dark:bg-white/5 text-left">
                        <span className="flex-1 text-left font-bold text-sm text-burgundy-900 dark:text-stone-200">QRIS</span>
                    ...
                    {/* BANK */}
                    <div className="border border-stone-200 dark:border-white/10 rounded-xl overflow-hidden">
                      <AnimatedButton onClick={() => { setOpenDropdown(openDropdown === 'BANK' ? null : 'BANK'); setSelectedPaymentMethod('BANK'); }} className="w-full p-4 flex justify-between items-center bg-white dark:bg-white/5 text-left">
                        <span className="flex-1 text-left font-bold text-sm text-burgundy-900 dark:text-stone-200">BANK (Atas Nama: DAFFA RAMDHANI)</span>
                    ...
                    {/* E-WALLET */}
                    <div className="border border-stone-200 dark:border-white/10 rounded-xl overflow-hidden">
                      <AnimatedButton onClick={() => { setOpenDropdown(openDropdown === 'E-WALLET' ? null : 'E-WALLET'); setSelectedPaymentMethod('E-WALLET'); }} className="w-full p-4 flex justify-between items-center bg-white dark:bg-white/5 text-left">
                        <span className="flex-1 text-left font-bold text-sm text-burgundy-900 dark:text-stone-200">E-WALLET (Atas Nama: DAFFA RAMDHANI)</span>
