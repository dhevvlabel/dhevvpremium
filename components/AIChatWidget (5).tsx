...
const AIChatWidget: React.FC<{ products: Product[] }> = ({ products }) => {
  ...
  const model = React.useMemo(() => {
    // Explicitly using 'v1beta' endpoint as requested
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const history = [
        {
          role: 'user',
          parts: [{ text: `SYSTEM INSTRUCTION: ${getSystemInstruction()}` }]
        },
        {
          role: 'model',
          parts: [{ text: "Siap Kak! Saya Dhevv AI, asisten virtual ramah Kakak. Ada yang bisa saya bantu hari ini?" }]
        },
        ...messages
