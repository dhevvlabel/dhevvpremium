...
const AIChatWidget: React.FC<{ products: Product[] }> = ({ products }) => {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // We put the system instruction at the very beginning of the history
      // using the 'user' role for maximum compatibility with older SDK versions
      const history = [
        {
          role: 'user',
          parts: [{ text: `Sistem: Kamu adalah asisten AI Dhevv Premium. Berikut adalah instruksi dan data produk kamu: ${getSystemInstruction()}` }]
        },
        {
          role: 'model',
          parts: [{ text: "Siap Kak! Saya Dhevv AI, asisten virtual Premium Kakak. Ada yang bisa saya bantu hari ini?" }]
        },
        ...messages
