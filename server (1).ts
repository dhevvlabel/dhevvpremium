...
async function startServer() {
  ...
  app.post("/api/send-receipt", async (req, res) => {
    try {
      ...
      const resendKey = rawKeys.find(k => k.startsWith('re_')) || rawKeys[0] || "";
      
      console.log(`[Server Resend Debug] Using key starting with: ${resendKey.substring(0, 5)}... Length: ${resendKey.length}`);
      ...
      if (error) {
        ...
        return res.status(400).json({ 
          ...
          details: error,
          hint: "Pesan ini muncul jika: 1. Domain belum Verify di Resend. 2. Kakak masih di Sandbox Mode. 3. API Key di AI Studio berbeda dengan Vercel.",
          name: error.name
        });
      }
