...
const getAIClient = () => {
  if (!aiClient) {
    ...
    // Check for actual empty key or common placeholder/broken strings
    ...
    if (isInvalid) {
      throw new Error("API Key Gemini belum terbaca di Vercel. Pastikan sudah menambahkan GEMINI_API_KEY di 'Environment Variables' dashboard Vercel dan redeploy.");
    }
...
const AIChatWidget: React.FC<AIChatWidgetProps> = ({ products }) => {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Prepare History for Context (Gemini Format)
